        ...
        <tbody>
            @if(count($products) == 0)
                ...
                hover:bg-gray-200 dark:border-neutral-500 dark:hover:bg-neutral-600">
                        <td colspan="4" class="whitespace-nowrap px-4 py-2 text-center">{{__('reports.no_data')}}</td>
                </tr>
            @endif
            @foreach ($products as $item)
                ...
                hover:bg-gray-200 dark:border-neutral-500 dark:hover:bg-neutral-600">
                    ...
                    <td class="whitespace-nowrap px-2 py-2">{{ getCategoryName($item->category_id) }}</td>
                    <td class="whitespace-nowrap px-2 py-2 flex flex-row items-center">
                        <div class="p-1">
                            ...
                            href="{{ route('products.show',$item->prefix) }}">{{__('products.show')}}</a>
                            
                            <button type="button" 
                            class="bg-red-600 text-xs text-white m-0 p-1.5 px-2 border border-white rounded-md dark:bg-red-900" 
                            wire:click ="delete('{{ $item->prefix }}')" >{{__('settings.form_delete')}}</buttom>
                        </div>
