<div class="bg-white rounded-xl p-4 dark:bg-neutral-800 mt-6 overflow-x-auto ">
    ...
    <thead class="border-b text-xs text-gray-400 bg-gray-100 
        dark:bg-gray-600 dark:text-gray-100 dark:border-neutral-500">
            <tr>
                <th scope="col" class="px-4 py-2 font-light w-[20%]">{{__('settings.form_order')}}</th>
                <th scope="col" class="px-4 py-2 font-light w-[60%]">{{__('settings.form_type')}}</th>
        ...
        <tbody>
            @if(count($types) == 0)
                <tr class="border-b border-stone-400">
                        <td colspan="3" class="whitespace-nowrap px-4 py-2 text-center">{{__('reports.no_data')}}</td>
                </tr>
            @endif
